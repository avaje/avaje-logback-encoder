 ...
 * formats stacktraces by doing the following:
 *
 * <ul>
 ...
 *   <li>Abbreviates class names based. See {@link #setShortenedClassNameLength(int)}.
 *   <li>Filters out consecutive unwanted stackTraceElements based on regular expressions. See
 *       {@link #excludes}.
 *   <li>Truncate individual stacktraces after any element matching one the configured regular
 *       expression. See {@link #truncateAfterPatterns}.
 *   <li>Uses evaluators to determine if the stacktrace should be logged. See {@link #evaluators}.
 ...
 * </ul>
 */
public final class ShortenedThrowableConverter extends ThrowableHandlingConverter {
  ...
  /** Abbreviator used to shorten the classnames. Initialized during {@link #start()}. */
  private Abbreviator abbreviator = new TrimPackageAbbreviator();

  /**
   * Patterns used to determine which stacktrace elements to exclude.
   *
   * <p>The strings being matched against are in the form "fullyQualifiedClassName.methodName" (e.g.
   * "java.lang.Object.toString").
   *
   * <p>Note that these elements will only be excluded if and only if more than one consecutive line
  ...
  private void parseOptions() {
    ...
    for (int i = 0; i < optionListSize; i++) {
      final String option = optionList.get(i);
      switch (i) {
        case OPTION_INDEX_MAX_DEPTH:
          setMaxDepthPerThrowable(
              parseIntegerOptionValue(
                  option,
        ...
        case OPTION_INDEX_MAX_LENGTH:
          setMaxLength(
              parseIntegerOptionValue(
                  option, FULL_MAX_LENGTH, SHORT_MAX_LENGTH, DEFAULT_MAX_LENGTH));
          break;
        default:
          /*
          ...
          switch (option) {
            ...
            default:
              ...
              final EventEvaluator<ILoggingEvent> evaluator =
                  evaluatorMap != null
  ...
  private int parseIntegerOptionValue(
      String option, int valueIfFull, int valueIfShort, int valueIfNonParsable) {
    ...
    if (OPTION_VALUE_SHORT.equals(option)) {
      ...
      try {
        ...
        addError(
            "Could not parse [" + option + "] as an integer, default to " + valueIfNonParsable);
    ...
    final StringBuilder builder =
        new StringBuilder(
            Math.min(
                ...
                this.maxLength + 100 > 0 ? this.maxLength + 100 : this.maxLength));
    if (rootCauseFirst) {
      appendRootCauseFirst(
          builder, null, ThrowableProxyUtil.REGULAR_EXCEPTION_INDENT, throwableProxy);
    } else {
      appendRootCauseLast(
          builder, null, ThrowableProxyUtil.REGULAR_EXCEPTION_INDENT, throwableProxy);
    }
   ...
   *
   * <p>The following values have special meaning:
   *
   * <ul>
   *   <li>{@code null} or empty string = no new line.
   *   <li>"{@code SYSTEM}" = operating system new line (default).
   *   <li>"{@code UNIX}" = unix line ending ({@code \n}).
   *   <li>"{@code WINDOWS}" = windows line ending {@code \r\n}).
   * </ul>
   *
   * <p>Any other value will be used as given as the lineSeparator.
   *
  ...
  /**
   * Return true if any evaluator returns true, indicating that the stack trace should not be
   * logged.
   */
  private boolean isExcludedByEvaluator(ILoggingEvent event) {
    for (final EventEvaluator<ILoggingEvent> evaluator : evaluators) {
      try {
        ...
        if (errors < CoreConstants.MAX_ERROR_COUNT) {
          addError(
              String.format("Exception thrown for evaluator named [%s]", evaluator.getName()), eex);
        } else if (errors == CoreConstants.MAX_ERROR_COUNT) {
          final ErrorStatus errorStatus =
              new ErrorStatus(
                  String.format("Exception thrown for evaluator named [%s]", evaluator.getName()),
                  this,
                  eex);
          errorStatus.add(
              new ErrorStatus(
  ...
  private void appendRootCauseLast(
      StringBuilder builder, String prefix, int indent, IThrowableProxy throwableProxy) {
    ...
    final IThrowableProxy[] suppressedThrowableProxies = throwableProxy.getSuppressed();
    if (suppressedThrowableProxies != null) {
      for (final IThrowableProxy suppressedThrowableProxy : suppressedThrowableProxies) {
  ...
  private void appendStackTraceElements(
      StringBuilder builder, int indent, IThrowableProxy throwableProxy) {
    if (builder.length() > this.maxLength) {
      return;
    }
    final StackTraceElementProxy[] stackTraceElements =
        throwableProxy.getStackTraceElementProxyArray();
    final int commonFrames = isOmitCommonFrames() ? throwableProxy.getCommonFrames() : 0;
    ...
    for (; i < stackTraceElements.length - commonFrames; i++) {
      ...
      final StackTraceElementProxy stackTraceElement = stackTraceElements[i];
      if (i < 1 || isIncluded(stackTraceElement)) { // First 2 frames are always included
        ...
        appendStackTraceElement(
            builder, indent, stackTraceElement, previousWrittenStackTraceElement);
    ...
    if (i + commonFrames < stackTraceElements.length) {
      ...
      if (consecutiveExcluded > 0) {
        ...
        appendStackTraceElement(
            builder, indent, stackTraceElements[i], previousWrittenStackTraceElement);
        appended++;
      }
  ...
  private void appendPlaceHolder(
      StringBuilder builder, int indent, int consecutiveExcluded, String message) {
  ...
  private void appendStackTraceElement(
      StringBuilder builder,
      int indent,
      StackTraceElementProxy step,
      StackTraceElementProxy previousStep) {
    if (builder.length() > this.maxLength) {
      return;
    }
    indent(builder, indent);

    final StackTraceElement stackTraceElement = step.getStackTraceElement();
   ...
   *
   * <p>Packaging data for the current step is only appended if it differs from the packaging data
   * from the previous step.
   */
  private boolean shouldAppendPackagingData(
      StackTraceElementProxy step, StackTraceElementProxy previousStep) {
  ...
  private void appendFirstLine(
    ...
    builder
        .append(abbreviator.abbreviate(throwableProxy.getClassName()))
        .append(": ")
